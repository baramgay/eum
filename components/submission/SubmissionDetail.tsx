...
export default function SubmissionDetail({ submissionId, role, onClose, onDecisionComplete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40">
      ...
      {/* 슬라이드오버 패널 */}
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-xl flex flex-col z-50">
        {/* 헤더 */}
        <div className="px-5 py-4 border-b bg-gray-50 flex items-start justify-between shrink-0">
          <div>
            {loading ? (
              <div className="h-5 w-48 bg-gray-200 rounded animate-pulse" />
            ) : detail ? (
              <>
                <h3 className="font-semibold text-gray-800 text-base leading-snug">{detail.meta.title}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLOR[detail.meta.status] ?? 'bg-gray-100 text-gray-600'}`}>
                    {STATUS_LABEL[detail.meta.status] ?? detail.meta.status}
                  </span>
                  <span className="text-xs text-gray-400 font-mono">{detail.meta.tenant_id}</span>
                </div>
              </>
            ) : null}
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none p-1 -mt-1">×</button>
        </div>
                ...
                <div className="p-5 space-y-5">
                  <dl className="grid grid-cols-2 gap-x-6 gap-y-3">
                    {[
                      ...
                      <div key={dt}>
                        <dt className="text-xs text-gray-400">{dt}</dt>
                        <dd className="text-sm text-gray-800 font-medium mt-0.5 break-all">{dd}</dd>
                      </div>
                    ))}
                  </dl>
                  ...
                  {detail.meta.description && (
                    <div>
                      ...
                      <p className="text-sm text-gray-700 bg-gray-50 rounded p-3">{detail.meta.description}</p>
                    </div>
                  )}
                  ...
                  {role === 'center' && (
                    <div className="border rounded-lg p-4 space-y-3 bg-gray-50">
                      <h4 className="text-sm font-semibold text-gray-700">심사 처리</h4>
                      <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">결정 메모 (사유)</label>
                        <textarea
                          value={note}
                      ...
                      {detail.meta.decision_note && (
                        <p className="text-xs text-gray-500">현재 결정 메모: {detail.meta.decision_note}</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {activeTab === 'preview' && (
                <div className="p-5">
                  {detail.preview.length === 0 ? (
                    <div className="text-center py-12 text-gray-400">업로드된 파일이 없습니다.</div>
                  ) : (
                    <>
                      ...
                      <div className="overflow-x-auto border rounded-lg">
                        <table className="w-full text-xs">
                          <thead className="bg-gray-50">
                            <tr>
                              {previewCols.map(c => (
                                <th key={c} className="px-3 py-2 text-left text-gray-600 font-medium whitespace-nowrap">{c}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {detail.preview.slice(0, 20).map((row, i) => (
                              <tr key={i} className="hover:bg-gray-50">
                                {previewCols.map(c => (
                                  <td key={c} className="px-3 py-1.5 text-gray-700 font-mono whitespace-nowrap">
              ...
              {activeTab === 'comments' && (
                <div className="p-5 space-y-4">
                  {detail.comments.length === 0 ? (
                    <div className="text-center py-8 text-gray-400 text-sm">코멘트가 없습니다.</div>
                  ) : (
                    <div className="space-y-3">
                      {detail.comments.map(c => (
                        <div key={c.comment_id} className="bg-gray-50 rounded-lg px-4 py-3 border">
                  ...
                  {role === 'center' && (
                    <div className="border-t pt-4 space-y-2">
                      <label className="block text-xs font-medium text-gray-600">새 코멘트</label>
                      <textarea
                        value={comment}
