...
export default function OpenApiClient() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-800">개방 API 관리</h2>
        <p className="text-sm text-gray-500 mt-0.5">공공데이터 외부 연계 API 키 발급 및 웹훅 설정</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: '활성 웹훅', value: stats.activeWebhooks, color: 'purple' },
            { label: '평균 응답(ms)', value: stats.avgResponseMs != null ? stats.avgResponseMs : '—', color: 'amber' },
          ].map(s => (
          ...
          {newPlain && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <code className="flex-1 text-xs bg-white border rounded px-2 py-1.5 font-mono text-green-900 break-all">
                  {newPlain}
                </code>
                <button
                  onClick={() => {
            ...
            <form onSubmit={createKey} className="bg-white rounded-lg border p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 text-white text-sm rounded-md disabled:opacity-60"
                >
                  {keyCreating ? '발급 중...' : '발급'}
                </button>
                <button
          ...
          <div className="space-y-2">
            {keys.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-base font-semibold text-gray-700 mb-1">발급된 API 키가 없습니다</h3>
                <p className="text-sm text-gray-400 mb-5 max-w-xs leading-relaxed">
                  API 키를 발급하면 외부 시스템에서 데이터에 접근할 수 있습니다
                </p>
                <button
        ...
        <div className="space-y-4">
          ...
          {showWHForm && (
            <form onSubmit={createWebhook} className="bg-white rounded-lg border p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">수신할 이벤트 *</label>
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  disabled={whCreating || !whEvents.length}
                  className="px-4 py-2 bg-blue-600 text-white text-sm rounded-md disabled:opacity-60"
                >
                  {whCreating ? '등록 중...' : '등록'}
                </button>
      ...
      {tab === 'docs' && (
        <div className="bg-white rounded-lg border shadow-sm p-6 space-y-6">
              ...
              {ENDPOINTS.map(ep => (
                <div key={ep.path} className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    {ep.params && (
                      <p className="text-xs text-gray-400 mt-0.5">파라미터: {ep.params}</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div>
            <h3 className="font-semibold text-gray-700 mb-2">인증 방법</h3>
...
curl -H "x-api-key: eum_xxxxxx" \\
  ...
  -o youth_pop.csv`}
          ...
          <div>
            <h3 className="font-semibold text-gray-700 mb-2">웹훅 이벤트</h3>
            <div className="space-y-1">
              {ALL_EVENTS.map(ev => (
                <div key={ev} className="flex items-center gap-2 text-xs">
                  <code className="bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded">{ev}</code>
                </div>
              ))}
            </div>
          </div>
