    ...
    <button
      ...
      className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-colors ${
        active
          ...
          : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-300 line-through hover:bg-gray-50 dark:hover:bg-gray-800'
      }`}
        ...
        <div ref={searchWrapRef} className="relative flex-1 min-w-0">
          ...
          {nodeSearch && (
            <Btn
              ...
              variant="ghost"
              size="sm"
              className="absolute right-1 top-1/2 -translate-y-1/2 !p-1 h-auto text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:hover:text-gray-400 transition-colors duration-150"
              aria-label="검색어 지우기"
              title="검색어 지우기"
            >
              <X className="w-4 h-4" />
            ...
            <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-900 border rounded-md shadow-lg max-h-60 overflow-y-auto">
              {searchSuggestions.map(n => (
                <button
                  ...
                  className="w-full px-3 py-2 text-left text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center justify-between transition-colors duration-150"
                >
        ...
        <div className="flex items-center gap-2 flex-wrap text-xs text-gray-500 dark:text-gray-400">
          ...
          {recentSearches.map(s => (
            <button
              ...
              className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-150"
            >
              {s}
            </button>
          ))}
          <button onClick={clearRecent} className="text-gray-400 dark:text-gray-300 hover:text-red-500 flex items-center gap-0.5 transition-colors duration-150">
                  ...
                  {sortedFilteredNodes.slice(0, 50).map(n => (
                    <tr
                      ...
                      className={`group cursor-pointer ${
                        selectedNode?.obj_id === n.obj_id ? 'bg-indigo-50' : 'hover:bg-gray-50 dark:hover:bg-gray-800'
                      }`}
