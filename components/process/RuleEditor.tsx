...
import { Btn, Badge, Input, Select, Textarea } from '@/components/ui'
import {
...
function RuleForm({ rule, onChange }: { rule: Rule; onChange: (r: Rule) => void }) {
  switch (rule.type) {
    ...
    case 'rename':
      return (
        <div className="flex gap-1 text-xs">
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="기존 컬럼명" value={rule.from}
            onChange={e => onChange({ ...rule, from: e.target.value })} />
          <span className="self-center">→</span>
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="새 컬럼명" value={rule.to}
    ...
    case 'cast':
      return (
        <div className="flex gap-1 text-xs">
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="컬럼명" value={rule.column}
            onChange={e => onChange({ ...rule, column: e.target.value })} />
          <Select className="border rounded px-2 py-1 bg-white dark:bg-gray-800" value={rule.to}
    ...
    case 'nullfill':
      return (
        <div className="flex gap-1 text-xs">
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="컬럼명" value={rule.column}
            onChange={e => onChange({ ...rule, column: e.target.value })} />
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="채울 값" value={String(rule.value ?? '')}
            onChange={e => onChange({ ...rule, value: e.target.value })} />
        </div>
      )
    case 'nulldrop':
      return (
        <Input className="border rounded px-2 py-1 text-xs w-full bg-white dark:bg-gray-800" placeholder="컬럼명 (쉼표 구분)"
          value={rule.columns.join(',')}
          onChange={e => onChange({ ...rule, columns: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })} />
      )
    case 'filter':
      return (
        <div className="flex gap-1 text-xs">
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="컬럼명" value={rule.column}
            onChange={e => onChange({ ...rule, column: e.target.value })} />
          <Select className="border rounded px-2 py-1 bg-white dark:bg-gray-800" value={rule.op}
          ...
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="비교값" value={String(rule.value ?? '')}
            onChange={e => onChange({ ...rule, value: e.target.value })} />
        </div>
      )
    case 'normalize':
      return (
        <div className="flex gap-1 text-xs">
          <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="컬럼명" value={rule.column}
            onChange={e => onChange({ ...rule, column: e.target.value })} />
          <Select className="border rounded px-2 py-1 bg-white dark:bg-gray-800" value={rule.fn}
      ...
      return (
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="소스 컬럼" value={rule.source}
              onChange={e => onChange({ ...rule, source: e.target.value })} />
            <Select className="border rounded px-2 py-1 bg-white dark:bg-gray-800"
              value={rule.expr}
            ...
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="대상 컬럼명" value={rule.target}
              onChange={e => onChange({ ...rule, target: e.target.value })} />
          </div>
          {needsFormat && (
            <Input className="border rounded px-2 py-1 w-full bg-white dark:bg-gray-800" placeholder="형식 예: YYYY-MM-DD"
    ...
    case 'dedup':
      return (
        <div className="flex flex-col gap-1 text-xs">
          <Input className="border rounded px-2 py-1 w-full bg-white dark:bg-gray-800"
    ...
    case 'codemap': {
      ...
      return (
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="대상 컬럼"
              value={rule.column}
              onChange={e => onChange({ ...rule, column: e.target.value })} />
            <Select className="border rounded px-2 py-1 bg-white dark:bg-gray-800"
          ...
          <Textarea className="border rounded px-2 py-1 w-full font-mono bg-white dark:bg-gray-800" rows={4}
    ...
    case 'concat':
      return (
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="결과 컬럼명"
              value={rule.target}
              onChange={e => onChange({ ...rule, target: e.target.value })} />
            <Input className="border rounded px-2 py-1 w-20 bg-white dark:bg-gray-800" placeholder="구분자"
    ...
    case 'split':
      return (
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="분리할 컬럼"
              value={rule.column}
              onChange={e => onChange({ ...rule, column: e.target.value })} />
            <Input className="border rounded px-2 py-1 w-20 bg-white dark:bg-gray-800" placeholder="구분자"
              value={rule.separator}
      ...
      return (
        <div className="flex flex-col gap-1 text-xs">
          ...
          <div className="flex gap-1">
            ...
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="집계할 컬럼"
              value={rule.column}
              onChange={e => onChange({ ...rule, column: e.target.value })} />
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="결과 컬럼명 (비우면 자동)"
              value={rule.target ?? ''}
              onChange={e => onChange({ ...rule, target: e.target.value })} />
          </div>
        </div>
    ...
    case 'join':
      return (
        <div className="flex flex-col gap-1 text-xs">
          ...
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="조인할 데이터셋 ID"
          ...
          <Input className="border rounded px-2 py-1 w-full bg-white dark:bg-gray-800" placeholder="조인 키 컬럼"
            value={rule.on}
    ...
    case 'pivot':
      return (
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="인덱스 컬럼"
              value={rule.index}
              onChange={e => onChange({ ...rule, index: e.target.value })} />
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="피벗 컬럼 (새 헤더)"
              value={rule.columns}
              onChange={e => onChange({ ...rule, columns: e.target.value })} />
          </div>
          <div className="flex gap-1">
            <Input className="border rounded px-2 py-1 flex-1 bg-white dark:bg-gray-800" placeholder="값 컬럼"
              value={rule.values}
              ...
              <div key={i} className={`border rounded p-3 ${isInvalid ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-gray-50 dark:bg-gray-950'}`}>
                ...
                <details className="mt-2 border-t pt-2">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5 items-center text-xs">
                    <Input
                      className="border rounded px-2 py-1 w-28 bg-white dark:bg-gray-800"
                      placeholder="컬럼명"
